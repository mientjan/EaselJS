 ...
 * @class Texture
 */
export class BaseTexture implements ILoadable<BaseTexture>
{
	public static createFromString(source:string, autoload:boolean = false):BaseTexture
	{
	...
	private _loadPromise:Promise<BaseTexture> = null;
	...
	public load(onProgress?:(progress:number) => any):Promise<BaseTexture>
	{
		if(!this._hasLoaded)
		{
			if(!this._loadPromise)
			{
				this._loadPromise = new Promise<BaseTexture>((resolve:(result:BaseTexture) => any, reject:() => any) =>
					this._load((scope) =>
					{
						resolve(scope);
	...
	protected _load(onComplete:(result:BaseTexture) => any, onError?:() => any):void
	{
	...
	public bindTexture(ctx:WebGLRenderingContext)
	{
		...
		if(this.hasLoaded())
		{
			...
			{
				if(console && console.warn) console.warn(`BaseTexture ${this.width}x${this.height} is not power of 2`, this)
			}
